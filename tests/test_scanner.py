...
def test_evm_positive():
    ...
    findings = scanner.scan_file("tests/mock_malicious.py")
...
def test_negative_control_clean_file():
    scanner = MomusScanner()
    findings = scanner.scan_file("tests/fixture_clean.py")
...
def test_allowlist_matches_by_substring_documented_behavior():
    with open("tests/allowlist.json", "r") as f:
        allowlist = json.load(f)
    
    scanner_no = MomusScanner()
    findings_no = scanner_no.scan_file("tests/mock_malicious.py")
    
    scanner_yes = MomusScanner(allowlist=allowlist)
    findings_yes = scanner_yes.scan_file("tests/mock_malicious.py")
...
def test_scan_diff_findings():
    scanner = MomusScanner()
    with open("tests/mock_diff.patch", "r") as f:
        diff_content = f.read()
    findings = scanner.scan_diff(diff_content, "tests/mock_diff.patch")
...
def test_scan_diff_removal_no_findings():
    scanner = MomusScanner()
    with open("tests/mock_removal.patch", "r") as f:
        diff_content = f.read()
    findings = scanner.scan_diff(diff_content, "tests/mock_removal.patch")
    assert len(findings) == 0
